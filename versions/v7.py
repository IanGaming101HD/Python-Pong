        ...
        if not gameStarted:
            ...
            if ball.left <= 0  or ball.right >= screenWidth or ball.top <= 0 or ball.bottom >= screenHeight or (ball.left <= player1.right and ball.bottom >= player1.top and ball.top <= player1.bottom) or (ball.right >= player2.left and ball.bottom >= player2.top and ball.top <= player2.bottom):
                if ball.left <= 0 or ball.right >= screenWidth:
                    ...
                    if ball.top <= 0:
                        ballDirection = normaliseAngle(random.choice([315, 405]))
                    elif ball.bottom >= screenHeight:
                        ballDirection = normaliseAngle(random.choice([135, 225]))
                    elif ball.left <= player1.right and ball.bottom >= player1.top and ball.top <= player1.bottom:
                        ballDirection = normaliseAngle(random.choice([45, 135]))
                    elif ball.right >= player2.left and ball.bottom >= player2.top and ball.top <= player2.bottom:
                        ballDirection = normaliseAngle(random.choice([225, 315]))
                    ballSpeed += 0.5
